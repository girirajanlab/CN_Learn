...
sql_query <- paste0("select CHR, START, END, CNV_TYPE, SAMPLE, ", dyn_query_string, "OVERLAP_COUNT, GRP_NAME, count(*) as NUM_TARGETS, count(*)/2 as HALF_NUM_TARGETS, ",
                     "CASE WHEN count(*)/2 > 0 THEN min(UNIQ_ID_TARGETS) - count(*)/2 ELSE min(UNIQ_ID_TARGETS) - 1 END as LEFT_FLANK_START, ",
                     ...
                     "CASE WHEN count(*)/2 > 0 THEN max(UNIQ_ID_TARGETS) + count(*)/2 ELSE max(UNIQ_ID_TARGETS) + 1 END as RIGHT_FLANK_END ",
